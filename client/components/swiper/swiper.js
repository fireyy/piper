...
class Swiper {
  constructor (options) {
    this._default = {
      container: '.piper-swiper',
      item: '.piper-swiper-item',
      direction: 'vertical',
      activeClass: 'active',
      threshold: 50,
      duration: 300,
    ...
    this.$container = this._options.container.querySelector('.piper-swiper')
  ...
  _itemDestoy () {
    for (let i = this.$items.length - 1; i >= 0; i--) {
      this.$items[i].removeEventListener('webkitTransitionEnd', this.transitionEndHandler, false)
    }
  }
  destroy () {
    this.stop()
